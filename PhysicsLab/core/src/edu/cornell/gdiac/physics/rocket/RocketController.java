...
public class RocketController extends WorldController implements ContactListener {
	...
	private boolean gravity_direction = false;
	...
	private void populateLevel() {
		...
		for (int ii = 0; ii < boxjv.size; ii += 2) {
			...
			box.setDensity(crates.getFloat( "density", 0 ));
			box.setFriction(crates.getFloat( "friction", 0 ));
	...
	public void update(float dt) {
		...
		// Creative Part
		Vector2 gravity = new Vector2(0, 4.9f);
		if(!gravity_direction){
			gravity = gravity.scl(-1);
		}
